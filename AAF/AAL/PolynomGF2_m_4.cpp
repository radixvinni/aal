...
//------------------------------------------------------------------------------
	...
	PolynomGF2_m_4& PolynomGF2_m_4::Add(PolynomGF2_m_4 &a, PolynomGF2_m_4 &b)
	{
        if(a.getModPolynom() != b.getModPolynom() || a.getModule() != b.getModule())
                        throw new Exception("модул€рные многочлены должны быть равны");
...
// простой метод возведени€ в степень 2 по модулю
  ...
  {
    if(n.isNegative())
        throw new Exception("„исло n - отрицательное результат не определен");
    if(n == 1)
    {
        *this = a;
        return *this;
