...
// ѕерестановка 0.. 255 построенна€ из цифр пи. Ёто дает "случайную" нелинейную операцию замены байта
...
// инициализаци€ MD2. Ќачинаем операцию MD2, записываем нов. контекст.
...
void MD2Update (MD2_CTX *context, unsigned char *input, unsigned int inputLen) /* контекст, входной блок, длина входного блока */
{
  ...
  /* ќновл€ем кол-во байт mod 16 */
  index = context->count;
  context->count = (index + inputLen) & 0xf;

  partLen = 16 - index;

  // ѕреобразуем столько раз,сколько возможно
  if (inputLen >= partLen) {
  ...
  /* ѕомещаем в буфер ост. часть вх. сообщени€ */
  MD2_memcpy
    ((POINTER)&context->buffer[index], (POINTER)&input[i], inputLen-i);
}

/* ќкончание MD2. «авершение операции преобразование,
записываем преобразованное сообщение и занул€ем контекст.
 */
